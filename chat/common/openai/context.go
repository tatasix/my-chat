...
func NumTokensFromMessages(messages []ChatModelMessage, model string) (numTokens int) {
	...
	for _, message := range messages {
		...
		//先采用简单估算法
		numTokens += len(message.Content)
		numTokens += len(message.Role)
...
// NumTokensFromMessagesV2 returns the number of tokens that will be used for a given model
func NumTokensFromMessagesV2(messages []ChatModelMessage, resultMessage, model string) (numTokens int64) {
	tkm, err := tiktoken.EncodingForModel(model)
	if err != nil {
		err = fmt.Errorf("EncodingForModel: %v", err)
		fmt.Println(err)
	...
	for _, message := range messages {
		...
		//numTokens += len(message.Content)
		//numTokens += len(message.Role)
		numTokens += int64(len(tkm.Encode(message.Content, nil, nil)))
		numTokens += int64(len(tkm.Encode(message.Role, nil, nil)))
	}
	numTokens += 3
	numTokens += int64(len(tkm.Encode(resultMessage, nil, nil)))
